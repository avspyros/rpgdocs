...
Missile" might appear in the spellbooks of a hundred different Magic-Users, yet each version would be unique in style and content.

{/* Elves do not use a spellbook but instead have an innate spellcasting ability. &#x2697; */}

### Access & Preparation

Each Magic-User starts with a spellbook containing the \*\*Read Magic spell and 3 random 1st-level spells. At each new level, they gain a random additional spell, which they must still research or
transcribe. Magic-Users can prepare spells according to the Magic-User table. Elves, however, begin only with Read Magic and gain one spell each level.

<Aside title="Optional:" icon="pencil">
  A Magic-User can choose one extra first level spell as a specialization spell. That spell can never change. It must always be one of the prepared spells. This happens only for first level
  Magic-Users and with first level spells. The rule can be used to boost 1st level characters.
</Aside>
...
<Aside type="note" title="Optional rule— Innate Magic" icon="pencil">
  If you follow the optional elf rule: ["Innate Magic"](/characters/classes/elf)
...
Every time the laboratory is used, the Magic-User must make a saving throw vs Magic. If the roll is successful, the laboratory loses 1d20 × 100 sp in value due to used supplies. If the roll is failed,
wasted supplies and accidents reduce the value of the laboratory by 2d20 × 100 sp. On a natural 1, there is a dangerous explosion which destroys both 5d20 × 100 sp of the laboratory’s value and the
project being worked on, and in addition, anyone present suffers 1d10 damage (save vs Breath Weapon for half).
